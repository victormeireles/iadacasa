...
const TYPE_LABELS: Record<string, string> = {
  base: 'Base', module: 'Módulo', reusable_registration: 'Cadastro',
  standard_prompt: 'Prompt', global_rule: 'Regra global',
  checklist: 'Checklist', example: 'Exemplo', implementation_guide: 'Guia',
}
const TYPE_COLORS: Record<string, string> = {
  base: 'bg-[#DEEBE1] text-[#235139]', module: 'bg-[#F6E0D3] text-[#B25A38]',
  global_rule: 'bg-[#EDD7DB] text-[#6E2A38]', checklist: 'bg-[#DCEAE5] text-[#4E7E6F]',
  standard_prompt: 'bg-[#F9EFD6] text-[#C28C2A]', reusable_registration: 'bg-[#EDE3D2] text-[#443E35]',
  example: 'bg-[#F5EEE1] text-[#6F6657]', implementation_guide: 'bg-[#F5EEE1] text-[#6F6657]',
}
const STATUS_COLORS: Record<string, string> = {
  active: 'bg-[#DEEBE1] text-[#235139]', draft: 'bg-[#F5EEE1] text-[#6F6657]', archived: 'bg-[#EDE3D2] text-[#443E35]',
}

export default async function AdminBlocosPage() {
  const blocks = await getAllKnowledgeBlocks()

  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[#211E19] mb-1">Blocos de conhecimento</h1>
          <p className="text-[#6F6657]">{blocks.length} bloco{blocks.length !== 1 ? 's' : ''} cadastrado{blocks.length !== 1 ? 's' : ''}.</p>
        </div>
        <Link href="/admin/blocos/novo">
              ...
              <tr key={block.id} className={`hover:bg-[#FBF7F0] transition-colors ${idx < blocks.length - 1 ? 'border-b border-[#F5EEE1]' : ''}`}>
                ...
                <td className="px-5 py-3.5">
                  <div className="flex justify-end">
                    <Link href={`/admin/blocos/${block.id}`} className="flex h-8 w-8 items-center justify-center rounded-lg text-[#968C7B] hover:bg-[#F5EEE1] hover:text-[#235139] transition-colors">
